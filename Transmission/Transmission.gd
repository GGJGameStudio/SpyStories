...
func _process(delta):
	for alice_or_bob in cia :
		...
		if alice_or_bob.is_a_parent_of(package) && dist_cia < 50 && alice_or_bob.get_node("Sprite").acting :
		...
		for charlie_or_erin in kgb:
			...
			if charlie_or_erin.is_a_parent_of(package) && dist_kgb < 50 && charlie_or_erin.get_node("Sprite").acting :
			...
			if alice_or_bob.is_a_parent_of(package) && dist_cia_kgb < 50 && charlie_or_erin.get_node("Sprite").acting :
				alice_or_bob.remove_child(package)
				...
				if alice==alice_or_bob:
					...
					Input.start_joy_vibration(1, 0, 1, 1)
				alice_or_bob.paralyzed = 1
			# la cia vole au kgb
			if charlie_or_erin.is_a_parent_of(package) && dist_cia_kgb < 50 && alice_or_bob.get_node("Sprite").acting :
				charlie_or_erin.remove_child(package)
