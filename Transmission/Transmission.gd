...
func _process(delta):
	for alice_or_bob in cia :
		...
		for charlie_or_erin in kgb:
			...
			if charlie_or_erin.is_a_parent_of(package) && dist_kgb < 50 && charlie_or_erin.get_node("BuddySprite").is_acting:
			...
			if alice_or_bob.is_a_parent_of(package) && dist_cia_kgb < 50 && charlie_or_erin.get_node("BuddySprite").is_acting:
				alice_or_bob.remove_child(package)
			...
			# la cia vole au kgb
			if charlie_or_erin.is_a_parent_of(package) && dist_cia_kgb < 50 && alice_or_bob.get_node("BuddySprite").is_acting:
				charlie_or_erin.remove_child(package)
